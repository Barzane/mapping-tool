...
def add_link_to_network(edges, start_airport):
    ...
    if available_end_airports:
        ...
        end_airport = np.random.choice(available_end_airports,1,replace=False,p=p)[0]
...
def generate(nodes, m0 = 1):
    ...
    chosen_airports = np.random.choice(len(nodes), 2, replace=False)
    ...
    while running:
        ...
        if available_airports:
            ...
            #insert new row
            edges = np.insert(edges,edges.shape[0], 0, axis=0)
                    
            #insert new column
            edges = np.insert(edges,edges.shape[1], 0, axis=1)
        
            #choose a new airport from the available airports using a uniform distribution      
            new_airport = np.random.choice(available_airports, 1, replace=False)[0] #[0] because it returns a 1-d array
        
            #add it to the airports list
            chosen_airports = np.insert(chosen_airports,len(chosen_airports), new_airport)
